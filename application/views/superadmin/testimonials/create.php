 ...
 * Page Type             : View
 * Page Purpose         :  Create Testimonial
 * Controller Name     :  superadmin/Cms/testimonials/Create
 * Alias                      : projectname_/superadmin/Cms/testimonials/Create
 * Designed By           : 
 * Designed On           : --
 * Design Completed On  : --
 * Created By            : kavitha
 ...
 * Modified On          : 
 * Extra notes            :
 * Folder Path           : views/superadmin/testimonials/create.php
 */
?>
<!DOCTYPE html>
<html lang="en">
                ...
                <div class="details col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    ...
                    <div class="info col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="reg col-lg-8 col-md-8 col-sm-8 col-xs-12 col-lg-push-2 col-md-push-2 col-sm-push-3 col-xs-push-0">
                            <h4 class="text-left">Create Testimonial</h4>
                               ...
                               <div class="row">
                                       <div class="col-xs-6 col-sm-6 col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="test_title" id="test_title" class="form-control input-md floatlabel" placeholder="Enter Title" title="Enter Title" maxlength="60"autocomplete="off"/>
                                            </div>
                                        </div>
                                        <div class="col-xs-6 col-sm-6 col-md-6">
                                        <div class="form-group">
                                            <label>Username<sup>*</sup><span id="username_error"></span></label>
                                            <input type="text" name="username" id="username" class="form-control input-md floatlabel" placeholder="Enter User name" title="Enter User name"/>
                                        </div>
                                        </div>
                                </div>
                               <div class="row">
                                ...
                                <div class="row">
                                       <div class="col-xs-6 col-sm-6 col-md-6">
                                            <div class="form-group">
                                                ...
                                                <textarea name="comment" id="comment" class="form-control input-md floatlabel" title="Enter Your Comment"></textarea>
                                            </div>
                                        </div>
                                </div>
                                <div class="row">
...
<script type="text/javascript">
    $('#testimonial_insert_form').on('submit', function (p) {
        ...
        if(test_title_err!=''){$('#test_title_error').html('Enter title');}
           else{$('#test_title_error').html('');}
        var username=$('#username').val();
        var username_err=V1_funNameCheck(username);
        if(username_err!=''){$('#username_error').html('Enter user name');}
           else{$('#username_error').html('');}
        var commentdate=$('#commentdate').val();
        var commentdate_err=V1_funEmptyCheck(commentdate);
        if(commentdate_err!=''){$('#commentdate_error').html('Select date and time');}
           else{$('#commentdate_error').html('');}
        var comment=$('#comment').val();
        var comment_err=V1_funEmptyCheck(comment);
        if(comment_err!=''){$('#comment_error').html('Enter comment');}
       ...
       if(str==true)
       {
          $.ajax({
            ...
            cache: false,
            processData: false,
            url:"<?php echo SITE_ADMIN_LINK; ?>Cms/insertTestimonial",
