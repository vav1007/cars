 ...
 * Page Type             : View
 * Page Purpose         :  Create Advise
 * Controller Name     :  superadmin/Cms/advisoryboard/Create
 * Alias                      : projectname_/superadmin/Cms/advisoryboard/Create
 * Designed By           : 
 * Designed On           : --
 * Design Completed On  : --
 * Created By            : kavitha
 ...
 * Modified On          : 
 * Extra notes            :
 * Folder Path           : views/superadmin/advisoryboard/create.php
 */
?>
<!DOCTYPE html>
<html lang="en">
                ...
                <div class="details col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    ...
                    <div class="info col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <div class="reg col-lg-8 col-md-8 col-sm-8 col-xs-12 col-lg-push-2 col-md-push-2 col-sm-push-3 col-xs-push-0">
                            <h4 class="text-left">Create Advise</h4>
                            <form role="form" name="advise_insert_form" id="advise_insert_form" method="post">
                               <div class="row">
                                       <div class="col-xs-6 col-sm-6 col-md-6">
                                            <div class="form-group">
                                                <label>Name<sup>*</sup><span id='adv_name_error'></span></label>
                                                <input type="text" name="adv_name" id="adv_name" class="form-control input-md floatlabel" placeholder="Enter Name" title="Enter Name" maxlength="60"autocomplete="off"/>
                                            </div>
                                        </div>
                                        <div class="col-xs-6 col-sm-6 col-md-6">
                                        <div class="form-group">
                                            <label>Email<sup></sup><span id="email_error"></span></label>
                                            <input type="text" name="email" id="email" class="form-control input-md floatlabel" placeholder="Enter User name" title="Enter email id" maxlength="60"  autocomplete="off"/>
                                        </div>
                                        </div>
                                </div>
                               <div class="row">
                                        <div class="col-xs-6 col-sm-6 col-md-6">
                                            <div class="form-group">
                                                ...
                                                <input type="text" name="mobile" id="mobile" maxlength="10" class="form-control input-md floatlabel number_class" title="Enter mobile number" placeholder="Enter mobile number" />
                                            </div>
                                        </div>
                                        <div class="col-xs-6 col-sm-6 col-md-6">
                                            <div class="form-group">
                                                <label>Suggestion<sup>*</sup><span id='comment_error'></span></label>
                                                <textarea name="suggestion" id="suggestion" class="form-control input-md floatlabel" title="Enter Your Suggestion" placeholder="Enter Your Suggestion"></textarea>
                                            </div>
                                        </div>
                                ...
                                <div class="row">
                                       ...
                                       <div class="col-xs-6 col-sm-6 col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" name="commentdate" id="commentdate" class="form-control input-md floatlabel" placeholder="Select Date" title="Select Date"/>
                                        </div>
                                        </div>
...
$('#commentdate').datetimepicker({value:new Date(),minDate: new Date(),maxDate:extendDate,format:"d-m-Y H:i:s",minTime:'10',pickTime:true, timepicker: true,}).css({'color':'#000'});
</script>
...
var mobilepattern= /^[6-9]+[0-9]{9}$/;
    $('#advise_insert_form').on('submit', function (p) {
        p.preventDefault();
        str=true;
            var adv_name=$('#adv_name').val();
            var adv_name_err=V1_funNameCheck(adv_name);
            if(adv_name_err!=''){$('#adv_name_error').html('Enter name');}
               else{$('#adv_name_error').html('');}
            var email=$('#email').val();
            if(email!='' && !emailpattern.test(email))
            {
                var email_err='Enter valid email';
                $('#email_error').html('Enter valid email');
            }
            var mobile=$('#mobile').val();
            if(mobile!='' && !mobilepattern.test(mobile))
            {
                var mobile_err='Enter valid mobile number';
                $('#mobile_error').html('Enter valid mobile number');
            }
            var suggestion=$('#suggestion').val();
            var suggestion_err=V1_funEmptyCheck(suggestion);
            if(suggestion_err!=''){$('#comment_error').html('Enter suggestion');}
               else{$('#comment_error').html('');}
            var commentdate=$('#commentdate').val();
            var commentdate_err=V1_funEmptyCheck(commentdate);
            if(commentdate_err!=''){$('#commentdate_error').html('Enter advise creation date');}
               else{$('#commentdate_error').html('');}
       if(adv_name_err!='' ||  suggestion_err!='' || commentdate_err!='')
       {
          str=false;
       }
       if(str==true)
       {
          $.ajax({
            ...
            cache: false,
            processData: false,
            url:"<?php echo SITE_ADMIN_LINK; ?>Cms/insertAdvisory",
