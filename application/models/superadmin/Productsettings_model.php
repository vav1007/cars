 ...
 */
class Productsettings_model extends CI_Model{
    ...
    public function brandList($search=NULL)
    {
        ...
        $slider_folder=  base_url().UPLOADS.'brand/';
        ...
        $cols="id as sliderid,title as title,CONCAT('".$slider_folder."',icon) as sliderpath,CASE $case_status  END as sliderstatus,activation_status as status,DATE_FORMAT(created_date,'%b %d %Y %h:%i %p') as createddate";
    ...
    //Brand Details
    public function brandDetails($sliderid)
    {
         $response=array();
        $table_name='brand_tbl';
        $table_name=trim($table_name);
        $slider_folder=  base_url().UPLOADS.'brand/';
        $cols="id as sliderid,title as title,CONCAT('".$slider_folder."',icon) as sliderpath";
    ...
    public function shapeList($search=NULL)
    {
        $response=array();
        $table_name='shape_tbl';
        $table_name=trim($table_name);
        $slider_folder=  base_url().UPLOADS.'shape/';
        $name_search=$search['name'];
        $activation_search=$search['activation'];
        $case_status="activation_status WHEN 0 THEN 'In-active' WHEN 1 THEN 'Active'";
        $cols="id as sliderid,title as title,CONCAT('".$slider_folder."',icon) as sliderpath,CASE $case_status  END as sliderstatus,activation_status as status,DATE_FORMAT(created_date,'%b %d %Y %h:%i %p') as createddate";
    ...
    //Shape Details
    public function shapeDetails($sliderid)
    {
         $response=array();
        $table_name='shape_tbl';
        $table_name=trim($table_name);
    ...
    public function productTypeList($search=NULL){
        $response=array();
        $table_name='producttype_tbl';
        $table_name=trim($table_name);
        $where=array('activation_status !'=>4);
        $name_search=$search['name'];
        $activation_search=$search['activation'];
        $case_status="activation_status WHEN 0 THEN 'In-active' WHEN 1 THEN 'Active'";
        $cols="id as id,title as title,CASE $case_status  END as status_message,activation_status as status";
        $this->db->select($cols,false)->from($table_name);
        if(!empty($name_search)) { 
        ...
        if($error['code']==0) {
                ...
                $response[MESSAGE]=($count > 0)?'Success':'Fail';
                $response[DESCRIPTION]=($count > 0)?'Total '.$count.' results found':'NO results found';
                $response['producttype_result']=($count > 0)?$sql->result():array();
                $response['statistics']=array(
                    'currentcount'=>$count,
                    'total'=>$this->Crud->statistics($table_name,array()),
                    'inactive'=>$this->Crud->statistics($table_name,array('activation_status'=>0)),
    ...
    public function productTypeDetails($id)
    {
        $response=array();
        $common_where=array('id'=>$id,'activation_status !='=>5);
        $sql=  $this->db->select('id as id,title as title')->from('producttype_tbl')->where($common_where)->limit(1)->get();
        $error=  $this->db->error();
        $error_message=$error['message'];
        if($error['code']==0)
        {
                ...
                $response[MESSAGE]=($count > 0)?'Success':'Fail';
                $response[DESCRIPTION]=($count > 0)?'Success':'No results found';
                $response['producttype_result']=($count > 0)?$sql->row():array();
        }
        else
        {
            $response[CODE]=DB_ERROR_CODE;
    ...
    /* >> Color section module code start */
    public function colorList($search=NULL){
        $response=array();
        ...
        if($error['code']==0) {
                ...
                $response[MESSAGE]=($count > 0)?'Success':'Fail';
                $response[DESCRIPTION]=($count > 0)?'Total '.$count.' results found':'NO results found';
                $response['color_result']=($count > 0)?$sql->result():array();
                $response['statistics']=array(
                    'currentcount'=>$count,
                    'total'=>$this->Crud->statistics($table_name,array()),
                    'inactive'=>$this->Crud->statistics($table_name,array('activation_status'=>0)),
    ...
    public function colorDetails($id)
    {
        $response=array();
        $common_where=array('id'=>$id,'activation_status !='=>5);
        $sql=  $this->db->select('id as id,title as title')->from('color_tbl')->where($common_where)->limit(1)->get();
        $error=  $this->db->error();
        $error_message=$error['message'];
        if($error['code']==0)
        {
                ...
                $response[MESSAGE]=($count > 0)?'Success':'Fail';
                $response[DESCRIPTION]=($count > 0)?'Success':'No results found';
                $response['color_result']=($count > 0)?$sql->row():array();
        }
        else
        {
            $response[CODE]=DB_ERROR_CODE;
    ...
    /*>> Color section module code end */
    /* >> Model section module code start */
    public function modelList($search=NULL){
        $response=array();
        $table_name='model_tbl';
        $table_name=trim($table_name);
        $where=array('activation_status !'=>4);
        $name_search=$search['name'];
        $activation_search=$search['activation'];
        ...
        if($error['code']==0) {
                ...
                $response[MESSAGE]=($count > 0)?'Success':'Fail';
                $response[DESCRIPTION]=($count > 0)?'Total '.$count.' results found':'NO results found';
                $response['model_result']=($count > 0)?$sql->result():array();
                $response['statistics']=array(
                    'currentcount'=>$count,
                    'total'=>$this->Crud->statistics($table_name,array()),
                    'inactive'=>$this->Crud->statistics($table_name,array('activation_status'=>0)),
    ...
    public function modelDetails($id)
    {
        $response=array();
        $common_where=array('id'=>$id,'activation_status !='=>5);
        $sql=  $this->db->select('id as id,title as title')->from('model_tbl')->where($common_where)->limit(1)->get();
        $error=  $this->db->error();
        $error_message=$error['message'];
        if($error['code']==0)
        {
                ...
                $response[MESSAGE]=($count > 0)?'Success':'Fail';
                $response[DESCRIPTION]=($count > 0)?'Success':'No results found';
                $response['model_result']=($count > 0)?$sql->row():array();
        }
        else
        {
            $response[CODE]=DB_ERROR_CODE;
